...
function renderPreviewTable(rows) {
    ...
    actionContainer.innerHTML = `
        <div id="signatureStatus" style="margin-bottom: 10px; color: #6b7280;">
            Calcul de la signature en cours...
        </div>
        <button id="goToDescriptionBtn" disabled style="
    ...
    const goToDescriptionBtn = document.getElementById('goToDescriptionBtn');

    // Attendre le calcul de signature avant d'autoriser la suite
    fetch(`${API_URL}/api/datasets/${datasetId}/compute-signature`, {
        ...
        .then(async (response) => {
            if (!response.ok) {
                ...
                throw new Error(errorData.detail || 'Erreur calcul signature');
            }
            return response.json();
        })
        .then(() => {
            signatureStatus.textContent = "Signature prête.";
            goToDescriptionBtn.disabled = false;
            goToDescriptionBtn.textContent = "Valider et passer à la description ➔";
        })
        .catch((err) => {
            console.warn("Calcul de signature indisponible:", err);
            signatureStatus.textContent = "Signature non disponible, mais la description pourra se recalculer à l’ouverture.";
            goToDescriptionBtn.disabled = false;
            goToDescriptionBtn.textContent = "Valider et passer à la description ➔";
        });

    // Ajouter l'événement de redirection au clic
    goToDescriptionBtn.addEventListener('click', () => {
